...
class Twipy:
	...
	def bot_setup(self, config_file="config.txt"):
		...
		# if they are old
		if (time.time() - os.path.getmtime(self.BOT_CONFIG["FOLLOWS_FILE"]) > 86400 or time.time() - os.path.getmtime(self.BOT_CONFIG["FOLLOWERS_FILE"]) > 86400):
	...
	def auto_follow(self, phrase, count=100, result_type="recent"):
		...
		for tweet in result["statuses"]:
			try:
				if (tweet["user"]["screen_name"] != self.BOT_CONFIG["TWITTER_HANDLE"] and tweet["user"]["id"] not in following and tweet["user"]["id"] not in do_not_follow):
	...
	def trends_auto_fav(self, count=100):
		"""
			favorite tweets that trends of istanbul
		"""
		
		results = self.TWITTER_CONNECTION.trends.place(_id=2344116) # WOEID http://woeid.rosselliot.co.nz
		trends = []
		...
		for trend in trends:
			self.wait_on_action()
			self.auto_fav(trend, count=count)